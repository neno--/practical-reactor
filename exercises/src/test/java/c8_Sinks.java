...
public class c8_Sinks extends SinksBase {
    ...
    public void single_subscriber() {
        ...
        Many sink = Sinks.many().replay().all();
        measurements = sink.asFlux();
        submitOperation(() -> {

            List<Integer> measures_readings = get_measures_readings(); //don't change this line
            measures_readings.forEach(integer -> sink.tryEmitNext(integer));
    ...
    public void it_gets_crowded() {
        //todo: feel free to change code as you need
        Flux<Integer> measurements = null;
        Many<Integer> many = Sinks.many().multicast().onBackpressureBuffer();
        measurements = many.asFlux();
        submitOperation(() -> {
            List<Integer> measures_readings = get_measures_readings(); //don't change this line
    ...
    public void open_24_7() {
        ...
        Sinks.Many<Integer> sink = Sinks.many().multicast().onBackpressureBuffer(SMALL_BUFFER_SIZE, false);
    ...
    public void emit_failure() {
        ...
        for (int i = 1; i <= 50; i++) {
            ...
            new Thread(() -> sink.emitNext(finalI, (signalType, emitResult) -> {
                System.out.println(signalType.toString() + " " + emitResult.toString());
                        return emitResult
