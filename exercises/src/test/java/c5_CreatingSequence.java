...
public class c5_CreatingSequence {
    ...
    public void repeat() {
        ...
        final Callable<Integer> incrementer = counter::incrementAndGet;

        Flux<Integer> repeated = Mono.fromCallable(incrementer).repeat(9); //todo: change this line
    ...
    public void generate_programmatically() {
        ...
        Flux<Integer> createFlux = Flux.create(sink -> {
            //todo: fix following code so it emits values from 0 to 5 and then completes
            Stream.of(1,2,3,4,5).forEach(sink::next);
            sink.complete();
        });
